...
const char * const GARLICCOIN_CONF_FILENAME = "garliccoin.conf";
const char * const GARLICCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/garliccoin/garliccoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //garliccoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", GARLICCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "garliccoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Garliccoin
    // macOS: ~/Library/Application Support/Garliccoin
    // Unix-like: ~/.garliccoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Garliccoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Garliccoin";
#else
    // Unix-like
    return pathRet / ".garliccoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GARLICCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Garliccoin Core copyright is not removed by accident
    if (copyright_devs.find("Garliccoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Garliccoin Core developers";
    }
