...
#include <qt/garliccoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Garliccoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new GarliccoinAddressEntryValidator(parent));
    widget->setCheckValidator(new GarliccoinAddressCheckValidator(parent));
}
...
bool parseGarliccoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no garliccoin: URI
    if(!uri.isValid() || uri.scheme() != QString("garliccoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!GarliccoinUnits::parse(GarliccoinUnits::GTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseGarliccoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseGarliccoinURI(uriInstance, out);
}

QString formatGarliccoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("garliccoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GarliccoinUnits::format(GarliccoinUnits::GTC, info.amount, false, GarliccoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openGarliccoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", GARLICCOIN_CONF_FILENAME));
    ...
    /* Open garliccoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Garliccoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Garliccoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Garliccoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Garliccoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "garliccoin.desktop";
    return GetAutostartDir() / strprintf("garliccoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a garliccoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Garliccoin\n";
        else
            optionFile << strprintf("Name=Garliccoin (%s)\n", chain);
