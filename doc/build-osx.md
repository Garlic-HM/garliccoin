...
This guide describes how to build garliccoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Garliccoin Core from source.
...
### 4. Clone Garliccoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Garliccoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `garliccoind` or  `garliccoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/garliccoin/garliccoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Garliccoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/garliccoin/garliccoin/issues/7714).
...
You can deploy a `.dmg` containing the Garliccoin Core application using `make deploy`.
...
## Building Garliccoin Core

### 1. Configuration

There are many ways to configure Garliccoin Core, here are a few common examples:
...
Run the following in your terminal to compile Garliccoin Core:
...
## Running Garliccoin Core

Garliccoin Core should now be available at `./src/garliccoind`.
If you compiled support for the GUI, it should be available at `./src/qt/garliccoin-qt`.

The first time you run `garliccoind` or `garliccoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Garliccoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Garliccoin"

touch "/Users/${USER}/Library/Application Support/Garliccoin/garliccoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Garliccoin/garliccoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Garliccoin/debug.log
```

## Other commands:

```shell
./src/garliccoind -daemon      # Starts the garliccoin daemon.
./src/garliccoin-cli --help    # Outputs a list of command-line options.
./src/garliccoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/garliccoin-qt -server # Starts the garliccoin-qt server mode, allows garliccoin-cli control
```
