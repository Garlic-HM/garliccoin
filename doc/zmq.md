...
The Garliccoin Core daemon can be configured to act as a trusted "border
router", implementing the garliccoin wire protocol and relay, making
...
The ZeroMQ feature in Garliccoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building garliccoind:
...
For instance:
    ...
    $ garliccoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/garliccoind.tx.raw \
...
These options can also be provided in garliccoin.conf.
...
From the perspective of garliccoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into garliccoind directly. Furthermore, no information is
...
using. Garliccoind appends an up-counting sequence number to each
